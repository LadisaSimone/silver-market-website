...
def fetch_silver_price() -> dict:
    # Reverted from Twelve Data's spot XAG/USD (2026-08-31 attempt) back to
    # yfinance/SI=F — Twelve Data's free tier does NOT include commodities
    # (confirmed on their pricing page; that requires their paid "Grow"
    # plan, ~$66-79/mo), which only surfaced as a 404 in production. A
    # verified, genuinely free spot XAG/USD source is still being
    # researched — see claude/website-roadmap.md in the project. Until
    # then this is COMEX futures again, correctly labeled as such on the
    # page (docs/index.template.html, src/render_static.py).
    return fetch_price(TICKER)
